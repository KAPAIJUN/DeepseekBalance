...
enum PlatformAPI {
    ...
    /// 轻量校验：拉取当月成本，成功返回 true
    static func validate(token: String) async -> Bool {
        let c = Calendar.current.dateComponents([.year, .month], from: Date())
        guard let y = c.year, let m = c.month else { return false }
        do {
            _ = try await fetchMonthCost(month: m, year: y, token: token)
            return true
        } catch {
            return false
        }
...
enum PlatformStore {
    nonisolated(unsafe) private static var memory: [String: PlatformDayUsage] = [:]
    nonisolated(unsafe) private static var loaded = false
    nonisolated(unsafe) private static var lastFetchByMonth: [String: Date] = [:]
    private static let lock = NSLock()
    nonisolated(unsafe) private static var lastErrorDescription: String? = nil
...
/// 剪贴板读取控制器（供输入弹窗复用，避免 Cmd+V 被前台限制拦截）
private final class PasteFieldController: NSObject {
    let field: NSTextField
    init(field: NSTextField) { self.field = field }
    @MainActor @objc func readPasteboard() {
...
enum PlatformSetup {
    ...
    static func promptAndSave(model: BalanceModel) {
        if let t = TextPrompt.prompt(
            title: "接入 DeepSeek 官网记录",
            informative: "浏览器登录 platform.deepseek.com → 开发者工具 Console 输入 localStorage.getItem('userToken') 回车，复制返回的字符串。\n可点「读取剪贴板」自动填入，或手动粘贴。Token 仅存本机 config.json（权限 600）。",
            placeholder: "粘贴 userToken…",
            okTitle: "保存并刷新") {
            model.savePlatformToken(t)
        }
    }
}
...
enum TextPrompt {
    ...
    static func prompt(title: String, informative: String, placeholder: String, okTitle: String = "保存") -> String? {
        ...
        let ctrl = PasteFieldController(field: field)
        pasteBtn.target = ctrl
        pasteBtn.action = #selector(PasteFieldController.readPasteboard)
...
enum TokenStatsParser {
    private static func makeISOParser(frac: Bool) -> ISO8601DateFormatter {
        let f = ISO8601DateFormatter()
        f.formatOptions = frac ? [.withInternetDateTime, .withFractionalSeconds] : [.withInternetDateTime]
        return f
    }

    /// 同时枚举 sessions 与 archived_sessions 下的会话文件
    private static func enumerateSessionFiles(_ fm: FileManager) -> [URL] {
        var urls: [URL] = []
        for root in [AppPaths.sessionsRoot, AppPaths.archivedSessionsRoot] {
    ...
    static func collectSessions() -> [SessionInfo] {
        ...
        for url in enumerateSessionFiles(fm) {
    ...
    private static func sessionMetaFromFilename(_ name: String) -> (id: String, date: Date)? {
        ...
        f.locale = Locale(identifier: "en_US_POSIX")
        f.calendar = Calendar(identifier: .gregorian)
        f.timeZone = .current // 文件名时间戳为本机本地时间
        guard let date = f.date(from: dateStr) else { return nil }
        return (id, date)
    }

    private static func sessionInfoFromFile(_ url: URL) -> (start: Date?, cwd: String?, end: Date, total: Int, input: Int, output: Int, active: TimeInterval)? {
        guard let handle = try? FileHandle(forReadingFrom: url) else { return nil }
        defer { try? handle.close() }

        let isoFormatter = makeISOParser(frac: true)
        let isoFormatterNoFrac = makeISOParser(frac: false)
        var cwd: String? = nil
        ...
        func process(_ line: Data) {
            ...
            if let ts = obj["timestamp"] as? String {
                if startRaw == nil { startRaw = ts } // 首个时间戳 = 会话开始
                lastTimestampRaw = ts
        ...
        let start = startRaw.flatMap { isoFormatter.date(from: $0) ?? isoFormatterNoFrac.date(from: $0) }
        return (start, cwd, end, total, input, output, active)
    }
...
enum AppPaths {
    ...
    static var archivedSessionsRoot: URL { codexHome.appendingPathComponent("archived_sessions") }
...
// MARK: - Token 统计缓存（跨线程安全，供后台解析使用）

private enum TokenStatCache {
    struct Entry { let mtime: Date; let tokens: Int }
    static let lock = NSLock()
    static let keyData = Data("total_token_usage".utf8)
    nonisolated(unsafe) static var entries: [String: Entry] = [:]
}
...
final class BalanceModel: ObservableObject {
    ...
    func refresh() async {
        ...
        if now.timeIntervalSince(lastTokenScan) >= tokenScanInterval {
            lastTokenScan = now
            consumedTokens = await Self.countConsumedTokensAsync()
        }
    ...
    func setInitialRecharge(_ amount: Double) {
        ...
        r.total = amount
        r.initial = amount
        r.last_balance = Double(toppedUp) // 重新基线化，避免后续 refresh 重复累计
    ...
    func savePlatformToken(_ t: String) {
        ...
        PlatformAPI.saveToken(trimmed)
        platformConnected = true // 先乐观显示，校验失败再回退
        Task {
            let ok = await PlatformAPI.validate(token: trimmed)
            if ok {
                await refresh()
            } else {
                platformConnected = false
                errorMessage = "平台 Token 无效或已过期，请重新接入"
                writeState(extra: ["error": errorMessage ?? ""])
            }
        }
    }
    ...
    func clearAPIKey() {
        removeConfigKey("deepseek_api_key")
    ...
    func clearPlatformToken() {
        removeConfigKey("deepseek_platform_token")
        platformConnected = false
        errorMessage = nil
    }
    ...
    private func removeConfigKey(_ key: String) {
        ...
        var cfg: [String: Any] = [:]
        if let data = try? Data(contentsOf: AppPaths.configFile),
           let json = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any] { cfg = json }
        cfg.removeValue(forKey: key)
        if let data = try? JSONSerialization.data(withJSONObject: cfg, options: [.prettyPrinted]) {
            try? data.write(to: AppPaths.configFile, options: [.atomic])
            try? FileManager.default.setAttributes([.posixPermissions: 0o600], ofItemAtPath: AppPaths.configFile.path)
        }
    ...
    // MARK: 消耗 token 统计（mtime 缓存，后台执行）

    nonisolated static func countConsumedTokensAsync() async -> String {
        await Task.detached(priority: .utility) {
            Self.formatCount(Self.countConsumedTokens())
        }.value
    }

    nonisolated static func countConsumedTokens() -> Int {
        let fm = FileManager.default
            ...
            for root in [AppPaths.sessionsRoot, AppPaths.archivedSessionsRoot] {
                ...
                for case let url as URL in en
                    ...
                    let mtime = (try? url.resourceValues(forKeys: [.contentModificationDateKey]))?.contentModificationDate
                        ?? Date.distantPast
                    TokenStatCache.lock.lock()
                    let cached = TokenStatCache.entries[path]
                    TokenStatCache.lock.unlock()
                    ...
                    TokenStatCache.lock.lock()
                    TokenStatCache.entries[path] = TokenStatCache.Entry(mtime: mtime, tokens: tokens)
                    TokenStatCache.lock.unlock()
                    total += tokens
                }
            }
            TokenStatCache.lock.lock()
            TokenStatCache.entries = TokenStatCache.entries.filter { fm.fileExists(atPath: $0.key) }
            TokenStatCache.lock.unlock()
            return total
        }
    }

    nonisolated private static func tokensInFile(_ url: URL) -> Int {
        ...
        func process(_ line: Data) {
            guard !line.isEmpty,
                  line.range(of: TokenStatCache.keyData) != nil,
...
struct TokenChartView: View {
    ...
    @State private var platformError: String? = nil          // 官网同步错误提示
    ...
    /// 分桶用日历：人民币模式用北京时区，Tokens 模式用本机时区
    private var bucketingCalendar: Calendar { showCNY ? Self.beijingCalendar : .current }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if loading {
                ...
                if showCNY {
                    ...
                    if let pe = platformError {
                        Text("⚠️ \(pe)（图表可能显示不完整）").font(.caption).foregroundColor(.red)
                    }
                }
                sessionList
            }
    ...
    private func silentRefresh() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                ...
                self.platformError = PlatformStore.lastError
            }
            Task {
    ...
    /// 底部消耗列表：按 token 消耗量从大到小排序
    private var sessionTokenList: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 0) {
                ForEach(inRangeSessions.sorted { $0.total > $1.total }) { s in
                    HStack(spacing: 8) {
    ...
    private func load() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                ...
                platformError = PlatformStore.lastError
                hoveredDate = nil
                hoverX = nil
                loading = false
            }
...
final class DeepSeekBalanceApp: NSObject, NSApplicationDelegate {
    ...
    private var forceShowTimer: Timer? // 活跃时强制显示菜单栏图标
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if Self.runningInstanceExists() {
            let alert = NSAlert()
            alert.messageText = "DeepSeekBalance 已在运行"
            alert.informativeText = "菜单栏中已有实例，请勿重复启动。"
            alert.runModal()
            NSApp.terminate(nil)
            return
        }
        ...
        model.onTitleChanged = { [weak self] in
            ...
            self?.updateForceShowTimer()
        }
        model.start()
    }

    // MARK: 活跃模式强制显示菜单栏图标（避免被其他应用挤掉）

    private func updateForceShowTimer() {
        if model.isActive {
            if forceShowTimer == nil {
                forceShowIfHidden() // 进入活跃立即强制一次
                let t = Timer(timeInterval: 15, repeats: true) { [weak self] _ in
                    Task { @MainActor in self?.forceShowIfHidden() }
                }
                RunLoop.main.add(t, forMode: .common)
                forceShowTimer = t
            }
        } else {
            forceShowTimer?.invalidate()
            forceShowTimer = nil
        }
    }

    private func forceShowIfHidden() {
        guard popover?.isShown != true else { return }
        if isStatusItemLikelyHidden() {
            reassertStatusItem()
        }
    }

    /// 状态栏窗口被系统移到屏幕外或压缩为 0 视为隐藏
    private func isStatusItemLikelyHidden() -> Bool {
        guard let win = statusItem.button?.window else { return false }
        let f = win.frame
        if f.width <= 1 || f.height <= 1 { return true }
        let screenFrame = win.screen?.frame ?? NSScreen.main?.frame ?? .zero
        return !f.intersects(screenFrame)
    }

    /// 重建状态栏条目（新条目系统会优先分配可见空间）
    ...
    private static func runningInstanceExists() -> Bool {
    ...
    private func showContextMenu() {
        ...
        menu.addItem(withTitle: "清除 API Key…", action: #selector(clearAPIKey), keyEquivalent: "")
        menu.addItem(withTitle: "清除官网记录…", action: #selector(clearPlatformToken), keyEquivalent: "")
    ...
    @objc private func setInitialRecharge() {
        if let s = TextPrompt.prompt(
            ...
            if let amount = Double(cleaned), amount >= 0 {
                ...
                alert.informativeText = "请输入非负数字金额，例如 20 或 50.5"
                alert.runModal()
            }
        }
    }
    ...
    @objc private func clearAPIKey() {
        let alert = NSAlert()
        alert.messageText = "清除 API Key？"
        alert.informativeText = "将删除本机保存的 DeepSeek API Key。"
    ...
    @objc private func clearPlatformToken() {
        let alert = NSAlert()
        alert.messageText = "清除官网记录 Token？"
        alert.informativeText = "将删除本机保存的平台 userToken，人民币图表将回退为余额估算。"
        alert.addButton(withTitle: "清除")
        alert.addButton(withTitle: "取消")
        if alert.runModal() == .alertFirstButtonReturn {
